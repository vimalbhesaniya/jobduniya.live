...
const Body = ({ style }) => {
  ...
  const handleHide = useCallback(
    (key, e) => {
      setHide((prev) => {
        setFilterData(
          Object.entries(e).filter(([key, _]) => jobSchemaKeys.includes(key))
        );
  ...
  const apps = api.useGetRequest({
    PATH: `fetchAppliedJobs/${id}`,
    enabled: true,
    initialData: [],
    key: ["applied-jos"],
  });

  console.log("call--apps", apps.data);
  ...
  useEffect(() => {
    ...
    setData(toggle ? apps.data : jobs.data);
  }, [toggle]);
      ...
      <div className={style}>
        <div className={css.TableContainer}>
          ...
          <table className="table table-hover  table-responsive-md  align-middle mb-0 ">
            <thead className="">
              <tr className="w-25 ">
                <td className="text-success fs-2 ">
                  {" "}
                  {toggle ? "Applied jobs" : "Saved jobs"} {data?.length}
                  {}
                </td>
              </tr>
            </thead>
                    ...
                    <tr>
                      <th className="w-50">
                        <div class="d-flex align-items-center">
                          <div class="">
                            <p class="fw-bold fs-3">{e?.jobId?.Title ?? ""}</p>
                      ...
                      <th className="text-center">
                        <div className="d-flex justify-content-center gap-2   align-items-end flex-column ">
                          {toggle ? (
                            ...
                            >
                              <i class="fa-solid fa-file-pen"></i> Cancle
                              Application
                            </button>
                          ) : (
                            <button
